...
- 一份轻量运行记录，把“计划怎么做”和“实际做了什么”对齐；确认过的系统缺陷可以沉淀为回归样本。
...
flowchart TB
  ...
  CONFIRM --> RUN["project_graph + run_record\n计划与实际执行证据"]
  RUN -->|没有可用业务图谱| ONBOARD["hs-onboarding\n生成第一版业务框架与待补清单"]
...
一句话理解：`hs-entry` 决定怎么做并建立运行记录，`hs-onboarding / hs-graph` 维护你理解业务所需的资产，`hs-analysis / hs-research` 完成判断，`hs-data-contract / hs-metric-audit / hs-table-builder` 保障数据型任务可靠，`hs-output` 负责把结论讲清，`hs-feedback` 用运行证据定位问题，经确认后回写并验证。
...
```bash
git clone https://github.com/bleo19870214-tech/hs-data-assistant.git
...
| `hs-feedback` | Bad Case 证据复盘、根因路由、确认后回写与回归验证 |
...
当前为 `v0.2.1` 公开测试版本。更新见 [CHANGELOG.md](CHANGELOG.md)。
