...
标准或重型任务开始前，必须读取 `project_graph` 与同目录的 `run_record`。资产变更计划、用户确认、执行结果、校验阻塞和用户纠正必须按 `../hs-entry/references/run-record-contract.md` 留下最小事件。业务资产的完整定义仍写入 business graph，不能用运行记录替代。
